...
[`docs/rhaydus/0.3.1/architecture.md`](../rhaydus/0.3.1/architecture.md) (§1–3, §5, §8–10). Read it
...
| `core:designsystem` | **Tokens only.** Material 3 theme, editorial typography, the icon/illustration catalogs, modifiers, shared-element scopes. Holds **zero project dependencies** — asserted by `checkModuleGraph`, with a scoped detekt `ForbiddenImport` closing the source-level half (migration tracker § 6, G2). The last components left in S4-5b; see the three rows below for where they went |
