...
- [ ] Desktop text/UI scaling: a user on Ubuntu/Linux reports the app text renders very small. Investigate desktop density / system-DPI handling so text matches platform expectations (not yet investigated).
- [ ] Desktop auto-updater: add a self-update mechanism for the desktop app so users don't have to manually download and overwrite each release (not yet investigated).
