...
Single long scrollable spread aggregating 7.4–7.8 + the existing 4 hero stats. Pushed from Profile. The scope selector every section reads from is Step 7.17, picked up immediately after this one. *(C.3)*
...
### Step 7.15 — "How you read" stats (M, depends on 3.13 + 7.9 + 7.17)
A Stats Atlas section driven by the graded traits (Step 3.13) and moods (Step 3.10) — the shape of the reading rather than its demographics. Surfaces: the plot-vs-character balance across the scope ("your year leaned character-driven — 69% of the books you graded"), the pace mix, the writing-style mix, and a mood profile (the moods reached for most, ranked by average intensity). Editorial register throughout — italic framing sentence, hairline bars, no dashboard chrome. Only counts books the user actually graded, and says so in the framing line rather than silently treating ungraded books as neutral. Scopes with the Atlas scope selector (Step 7.17); feeds Step 7.14. *(B.5.16)*

### Step 7.16 — Publication & provenance stats (S–M, depends on 7.9 + 7.17, + 3.9 for nationality, + 3.14 for source)
The classic reading-journal spread, most of it computable from data already held. Surfaces: **author nationality** (from Step 3.9's country-of-birth tag), **language + share translated** (from the edition's language field), **year-published distribution** (from the release date — separates a new-releases reader from a backlist reader), **audience mix** (YA / Middle Grade / Adult, on top of Step 6.14's classification), and **where you got it** (from Step 3.14). Format split (Step 7.6) and rating + genre distributions (Step 7.4) already exist — this closes the set so the Atlas covers everything a paper reading journal would. Each sub-stat degrades independently: a user who never tagged nationality still gets the language and year charts. Scopes with the Atlas scope selector (Step 7.17); feeds Step 7.14. *(B.5.17)*

### Step 7.17 — Stats Atlas scope selector (M, depends on 7.9; blocks 7.13 / 7.15 / 7.16)
*Numbered last, picked up early — take this straight after Step 7.9, before the three stat sections that depend on it.* One scope control at the head of the Atlas that every section below it reads from, replacing the "year filter" 7.13 / 7.15 / 7.16 and the contribution graph were all written against without anything defining it. Two kinds of scope in one picker: **rolling windows** (last 30 days, last 6 months, last 12 months) and **static periods** (each calendar year the user has reading data for, plus "all time"). Editorial register — a row of eyebrow-cased chips under the Atlas masthead with a quiet italic "showing 2026" line, not a Material filter bar or a dropdown. The selection is a single piece of Atlas state threaded into every section's query, so no section computes its own window; a section with no data in the chosen scope says so in its framing sentence rather than rendering an empty chart, and the picker offers only years the user actually read in. The scope type and its date-range resolution are shared with Step 7.14's wrap-up scope sheet rather than duplicated — one vocabulary, one range type, two surfaces. The contribution graph (Step 7.12 / B.5.18) follows the static-year periods only, being a full-year form. *(B.5.19)*

### Step 7.18 — Full genre breakdown (S–M, depends on 7.4; composes with 7.9 + 7.17)
Step 7.4's genre chart shows the top 5; this adds the explicit route to *all* of them, for the reader who wants the answer rather than the summary. The cap is still a **data-layer** decision — `toGenreBreakdown` in `:core:profile` takes the top 5 before mapping — so the first move is lifting it into presentation: `GenreBreakdown` carries the complete ranking, the Profile genre section keeps rendering its top 5 off it, and no surface needs a second query to go deeper. Then a quiet expand affordance on the genre section opens the full ranking — every genre the scope's finished books carry, ordered by share, in the same ranked-share-bar register — on its own scrollable surface with a count in the framing line rather than an unbounded inline list. Scopes with the Atlas scope selector (Step 7.17), and reachable from the Profile genre section as well as from the Atlas section, so the answer isn't gated behind the Atlas.

Three things 3.1.1 already settled, so don't re-open them: the server-side `limit: 5` on `taggable_counts` is **gone** (this step was structurally impossible while the client could only ever see five genres per book); the shares are of genre-tagged **books**, not assignments; and untagged books are excluded from that denominator. Carry both caveats through the expansion — a book carries several genres, so the shares overlap, do not sum to 100, and the full view must not add a total or an "everything else" remainder row. The framing line's book count must be the **genre-tagged** count (`GenreBreakdown.taggedBookCount`), not every finished book, or it implies untagged books were surveyed. *(B.5.20)*
