...
Step 7.4's genre chart shows the top 5; this adds the explicit route to *all* of them, for the reader who wants the answer rather than the summary. The cap is currently a **data-layer** decision — `toGenreSlices` in `:core:profile` takes the top 5 before mapping — so the first move is lifting it into presentation: the aggregate returns the complete ranking, the Profile genre stack keeps rendering its top 5 off it, and no surface needs a second query to go deeper. Then a quiet expand affordance on the genre section opens the full ranking — every genre the scope's finished books carry, ordered by share, in the same hairline-bar editorial register — on its own scrollable surface with a count in the framing line ("48 genres across 212 books") rather than an unbounded inline list. Scopes with the Atlas scope selector (Step 7.17), and reachable from the Profile genre stack as well as from the Atlas section, so the answer isn't gated behind the Atlas. Keep the share-of-assignments caveat intact through the expansion: a book carries several genres, so the shares do not sum to 100, and the full view must not add a total or an "everything else" remainder row. Lands in the same aggregation as the open "exclude books with no genre data" question in `now.md` — settle that with this step rather than opening `toGenreSlices` twice. *(B.5.20)*
