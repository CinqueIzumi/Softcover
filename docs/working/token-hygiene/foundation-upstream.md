...
| FU-3 | Session loop: `ACTIVE.md` + `## Now` convention, SessionStart resume hook, context-budget hook, `/handoff` skill | `.claude/hooks/session-resume.sh`, `.claude/hooks/context-budget.sh`, `.claude/skills/handoff/` | plugin hooks + skill; the convention in `docs/architecture.md` or a new `docs/working-docs.md` | done locally |
