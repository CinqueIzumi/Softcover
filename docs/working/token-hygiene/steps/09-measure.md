...
4. Decide on a `/write-docs` skill (mirroring `document-code` for markdown: type → destination → rewrite,
   not append → budget check). Propose it only if `doc-guard.sh` denials or reviewer 🟡 doc-hygiene findings
   show `.claude/rules/docs.md` plus the hook are not enough.
5. Close out:
