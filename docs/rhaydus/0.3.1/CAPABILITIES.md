...
- **layout/** — `rememberWindowSizeClass()` + `WindowSizeClass` / `WindowWidthClass` (the single breakpoint source: COMPACT < 600dp, MEDIUM 600-840dp, EXPANDED >= 840dp), `TwoPaneScaffold`, `ContentMaxWidth` + `Modifier.cappedContentWidth`, `BottomBarScaffold` + `BottomBarPlacement` (the bottom-bar host for both placements: `OVERLAY` measures a caller-supplied bar slot and provides `footprint + barSpacing`, `DOCKED` hosts it in a Material `Scaffold` and provides `barSpacing` alone), `LocalBottomBarPadding` + `rememberBottomBarPadding()` (the paired live read — always "the trailing padding scrolling content reserves", so screens never branch on the placement), `BottomNavigationSpacer`, `ExpandableFlowRow` (a `FlowRow` that collapses to N lines behind a "show more" affordance; `FlowRowExpansion.Progressive` reveals a few more lines per tap, `FlowRowExpansion.Full` reveals everything at once, and `collapsible` adds a "show less" back — both affordances are replaceable slots), spacers.
