...
| Piece | Role |
|---|---|
| Milestone `description` (on GitHub) | One release section of the public roadmap |
| `header.md` | The static intro and caveats block |
| `versionName` in `app/build.gradle.kts` | Fills the "Current release" line |
| `FOOTER` in `generate_roadmap.py` | The "Under consideration" tail |

`.github/workflows/roadmap.yml` runs this on any milestone change and commits the result to
`main`. The in-app Roadmap screen fetches the raw file at runtime, so a milestone copy edit
