...
> platform seam; see §4/§7). **P5 is complete:** `feature:library` is converted — mostly a `commonMain`
...
> tests already in `androidHostTest`). All iOS targets compile and `check` is green. `feature:reading` is
> converted too — a `commonMain` move whose only breakage was the **CMP-forced kotlinx-datetime 0.7.1 vs
> Android-0.6.2 `Clock`/`Instant` skew** (§7 / P4 `core:designsystem`): `ReadingScreen` and `StreakStrip`
> read "today" via `Clock.System.todayIn(...)` and the greeting's hour via
> `Clock.System.now().toLocalDateTime(...).hour`, none of which resolve in `commonMain` (the `Clock` type
> sits in `kotlinx.datetime` on Android's 0.6.2 but `kotlin.time` on iOS's 0.7.1). The four `todayIn`
> sites moved to the existing `currentLocalDate()` seam; the hour read drove a **new sibling
> `currentLocalDateTime()` seam** in `core:designsystem` (same `expect`/`actual` shape, same rationale —
> the only line differing between the actuals is the `Clock` import). Stable date types (`DateTimeUnit`/
> `LocalDate.minus`/`plus`) stayed in `commonMain` untouched; MockK tests already in `androidHostTest`.
> All iOS targets compile and `check` is green. `feature:explore` is converted too — the lone
> `android.content.Context` use the plan flagged turned out to be the search-history DataStore, so it
> followed the `core:preferences` okio template (not a "just `expect`-wrap the Context" job):
> `SearchHistorySerializer` became an `OkioSerializer` (`BufferedSource`/`BufferedSink`), the
> `Context.searchHistory` `dataStore` delegate became a `createSearchHistoryDataStore(fileSystem,
> producePath)` okio factory, and the `SearchHistoryDataStore` binding moved out of `exploreModule`
> into a new `expect val platformExploreModule` (pulled in via `includes(...)`) — Android supplies
> `filesDir/datastore/search_history.json` (store continuity preserved) with koin-android confined to
> `androidMain`, iOS supplies `NSDocumentDirectory`. The module's catalog dep swapped from the
...
> `check` is green. `feature:book_detail` is converted too — its two named blockers turned out to be
> mostly solved already: open-URL was the CMP `LocalUriHandler` (no work) and the lone `Toast` swapped
> to the existing `SnackBarManager`. The real platform seam was the **image share-sheet** — the
> `Intent.ACTION_SEND` chooser in `ShareBookBottomSheet` was promoted into the already-KMP
> `ShareCardCapture` (`core:designsystem`) as a new `share(displayName): ShareOutcome` action, so the
> feature itself ends up pure `commonMain` with **no `expect`/`actual` of its own**: the Android `actual`
> absorbs the `FileProvider` + `ACTION_SEND` chooser, and the iOS `actual` (previously a TODO stub) is now
> a real capture (`GraphicsLayer.toImageBitmap()` → skia PNG → `NSData`/`UIImage`) presented via
> `UIActivityViewController` — awaiting dismissal through `completionWithItemsHandler`, resolving the
> foreground `UIWindowScene`'s window, and forcing the GPU read-back onto the main dispatcher;
> `saveToGallery` stays an iOS `Failure` stub (Photos permission out of scope). The iOS gate caught three
> ordinary `commonMain` JVM-only leaks — `sortedSetOf(...)` → `mutableSetOf(...).sorted()`, three
> `"%.1f".format(rating)` → the existing `formatDecimalNumber` seam, and `Clock.System.todayIn(...)` →
> the existing `currentLocalDate()` seam (the §7 / `feature:reading` `Clock` skew) — plus one subtle one:
> `@Transient` on the two Voyager `Screen` constructors. On Android Voyager's `Screen` is
> `java.io.Serializable` and the default navigator serializes the back stack across config-change /
> process-death; `@Transient` excluded the non-serializable `BookInitialCover` / `BookEdition` morph hints
> from that, and it had been resolving only via the JVM's default `kotlin.jvm.*` import (absent on Native).
> Rather than silently drop it (a `NotSerializableException`-on-rotation regression), it drove a new
> **`TransientNavArg` `@OptionalExpectation` seam** in `core:designsystem` — Android `actual typealias` →
> `kotlin.jvm.Transient` (bytecode-verified the fields stay `transient`), iOS erased to a no-op — the
> first param-bearing `commonMain` Voyager screen, so it establishes the pattern for the rest of P5/P6.
> `build.gradle.kts` swapped `koin.compose` → `koin.compose.multiplatform`; `DESIGN_SYSTEM.md`'s share-card
> paragraph was updated (the share *intent* is now part of the capture contract, not the calling surface's
> job). All iOS targets compile and `check` is green. **P6 is complete on the Android + iOS path:**
> `:orchestration` is converted, the `OrchestrationKit` iOS framework is exported and links clean, the
> kotlinx-datetime 0.7.1 root-cause fix landed, and the `iosApp/` Xcode project now exists (committed) with
> its Softcover launcher icon + display name wired. **The desktop (JVM) target is also complete** (§7, last
> row): the `:desktopApp` Compose Desktop shell launches, with real storage, OS-keychain API-key storage,
> connectivity, notifications, and share-card export; the editorial fonts are now bundled cross-platform.
> All three platforms (Android, iOS, desktop) are migrated — the remaining per-platform stubs (barcode
> scanning, app-update, lock-screen session) are platform-N/A and consistent across iOS + desktop.
>
...
- **Date/time library:** **`kotlinx-datetime` 0.7.1** + `kotlin.time.Clock`/`Instant`/`Duration`.
  (Originally pinned 0.6.2; bumped to 0.7.1 at the P6 iOS entry point — the expressive material3 alpha
  forces 0.7.1 on Native, so the whole repo had to match it. See the P6 kotlinx-datetime row in §7.)
...
- **CMP vector drawables accept only literal `#` colors — `@color/`, `@android:color/`, and `?attr/`
  references crash at runtime.** Compose Multiplatform's own vector parser
  (`org.jetbrains.compose.resources.vector.parseColorValue`) does `require(color.startsWith("#"))`,
  so any `composeResources` vector whose `fillColor`/`strokeColor` is a resource/theme reference
  throws `IllegalArgumentException: Invalid color value …` the first time it is drawn — **not** at
  build time (`check` stays green; it only blows up on-device). The design-system CMP move (P4) left
  this latent in seven drawables: the two onboarding illustrations (`illu_writing`/`illu_sign_up`,
  `@color/m3_background` + `@color/secondary_container`, which were theme-attr-driven on Android) and
  five action icons (`ic_share`/`ic_sort`/`ic_filter_list`/`ic_drag_handle`/`ic_view_layout`,
  `@android:color/white`). Fixes: monochrome icons → a literal fill (`#e3e3e3`, matching the catalog;
  they're tinted at the Compose layer anyway). Theme-adaptive art → literal light values in
  `composeResources/drawable/` plus a `composeResources/drawable-dark/` variant with the dark values
  — CMP's `ThemeQualifier` selects `-dark` via `isSystemInDarkTheme()`, the same signal `SoftcoverTheme`
  uses, so they stay in sync and the single generated `Res.drawable.*` accessor needs no code change.
  The root `android:tint="?attr/…"` attribute is harmless — CMP ignores it. **When auditing a module's
  drawables for CMP, grep `(fillColor|strokeColor)="[^#]`.**
- **`:app` needs the Kotlin Compose compiler plugin once it binds a `@Composable` seam.** The
  build-type debug-routes binding (`app/src/debug/di/DebugRoutesModule.kt`) implements the
  `@Composable DebugRoutesContent` seam, but `:app` (application module — `android.application` +
  `kotlin.android`) did not apply `org.jetbrains.kotlin.plugin.compose`. Without it the
  `@Composable override fun Render()` compiles as a bare `Render()` (no `Composer`/`$changed`
  params), so invoking it through the interface — which `core:designsystem` compiled *with* Compose
  as `Render(Composer, int)` — throws `AbstractMethodError` at runtime (debug-build-only; never caught
  by `check`/`assembleDebug`, latent since P4). Fix: add `alias(libs.plugins.kotlin.compose)` to
  `:app`. The `AndroidComposeConventionPlugin` can't be reused here — it configures `LibraryExtension`,
  and `:app` is an application, not a library.
...
- **klib compilation ≠ link — the iOS `check` gate has a blind spot.** The per-module gate runs only
  `compileKotlinIos*`, which compiles each module's klib **in isolation against its own resolved
  dependency versions**. A transitive-version conflict that only exists when modules are *linked
  together* (e.g. a CMP module forcing kotlinx-datetime 0.7.1 onto a non-CMP module compiled against
  0.6.2) is invisible to it — Kotlin/Native **partial linkage** turns the unresolved symbols into
  runtime-throwing stubs at *link* time, not compile time, with only `i:` warnings. Nothing produced an
  iOS *binary* until the P6 framework export, so a foundation-wide datetime break sat latent behind every
  green "iOS compiles" check since P1 (see the §7 kotlinx-datetime row). **Lesson: a real
  `linkDebugFrameworkIos*` (or running on a simulator) is the only thing that proves the iOS app links;
  add it to CI alongside the compile gate, and treat partial-linkage `i:`/`uses unlinked` warnings as
  errors-in-waiting.**
...
| P5 | `feature:reading` | ✅ done — a `commonMain` move + `androidHostTest`; the one breakage was the CMP-forced kotlinx-datetime 0.7.1 vs Android-0.6.2 `Clock`/`Instant` skew (P4 `core:designsystem`): `ReadingScreen`/`StreakStrip` read "today" via `Clock.System.todayIn(...)` and the greeting hour via `Clock.System.now().toLocalDateTime(...).hour`, which don't resolve in `commonMain` (`Clock` is `kotlinx.datetime` on 0.6.2, `kotlin.time` on 0.7.1). The four `todayIn` sites → the existing `currentLocalDate()` seam; the hour read → a new sibling `currentLocalDateTime()` seam in `core:designsystem` (same `expect`/`actual` shape, only the `Clock` import differs between actuals). Stable date types (`DateTimeUnit`/`LocalDate.minus`/`plus`) stayed in `commonMain`; MockK tests already in `androidHostTest`. The notification-permission requester comes from `core:notification`'s common contract. All iOS targets compile; `check` green |
| P5 | `feature:explore` | ✅ done — the `android.content.Context` use the plan flagged was the search-history DataStore. Converted on the `core:preferences` okio-DataStore template: `SearchHistorySerializer` is now an `OkioSerializer` (`BufferedSource`/`BufferedSink`); the `Context.searchHistory` `dataStore` delegate became a `createSearchHistoryDataStore(fileSystem, producePath)` okio factory; the `SearchHistoryDataStore` binding moved out of `exploreModule` into a new `expect val platformExploreModule` (pulled in via `includes(...)`) — Android supplies `filesDir/datastore/search_history.json` (store continuity preserved) with koin-android confined to `androidMain`, iOS supplies `NSDocumentDirectory`. Catalog swap `libs.dataStore` (Android-only) → `datastore-core` + `datastore-core-okio` + `okio`. One more iOS breakage: `ExploreScreen`'s `"%.1f".format(rating)` (JVM-only) → the existing `core:designsystem` `formatDecimalNumber(value, fractionDigits)` seam. MockK serializer test migrated to okio `Buffer` in `androidHostTest`. All iOS targets compile; `check` green |
| P5 | `feature:settings` | ✅ done — a near-leaf `commonMain` move (settings + appearance + library-visibility TOAD screens/models/actions/use cases + Koin module), MockK tests → `androidHostTest`, `koin.compose.multiplatform` for the `koinInject`/`koinScreenModel` call sites. Two seams: (1) the `android.os.Build` dynamic-color check became a `supportsDynamicColor()` `expect`/`actual` (Android `Build.VERSION.SDK_INT >= S` with `@ChecksSdkIntAtLeast`, iOS `false`); (2) the `DebugRoutesContent` DI seam it consumes had to move `androidMain` → `commonMain` in `core:designsystem` — a `commonMain` consumer can't see a dependency's `androidMain` declaration, so the interface (not its `androidMain` impls) belongs in `commonMain` (see §2.3). All iOS targets compile; `:feature:settings:check` green |
| P5 | `feature:book_detail` | ✅ done — the plan's "external-link `Intent` + `Toast`" seam was smaller than predicted: open-URL was already CMP `LocalUriHandler` (no work), and the lone `Toast` → the existing `SnackBarManager`. The real seam was the **image share-sheet** (`Intent.ACTION_SEND`) in `ShareBookBottomSheet` — promoted into the already-KMP `ShareCardCapture` in `core:designsystem` as a new `share(displayName): ShareOutcome` action: the Android `actual` absorbs the `FileProvider`+`ACTION_SEND` chooser; the iOS `actual` is a **real** capture (`GraphicsLayer.toImageBitmap()` → skia PNG → `NSData`/`UIImage`) presented via `UIActivityViewController` (awaits dismissal through `completionWithItemsHandler`, foreground-`UIWindowScene` lookup, capture forced onto the main dispatcher); `saveToGallery` on iOS stays a `Failure` stub (Photos permission out of scope). Three `commonMain` JVM-only leaks the iOS gate caught: `sortedSetOf(...)` → `mutableSetOf(...).sorted()`; three `"%.1f".format(rating)` → the `formatDecimalNumber` seam; `Clock.System.todayIn(...)` → the `currentLocalDate()` seam. Plus a subtle one — `@Transient` on the two Voyager `Screen` constructors (Android's `Screen` is `Serializable` and the navigator serializes the back stack; the `BookInitialCover`/`BookEdition` morph hints aren't) resolved via JVM's default `kotlin.jvm.*` import, unavailable on Native → a new **`TransientNavArg` `@OptionalExpectation` seam** in `core:designsystem` (Android `typealias`→`kotlin.jvm.Transient`, iOS erased no-op; bytecode-verified the fields stay `transient`) — the first param-bearing `commonMain` Voyager screen, so it sets the pattern. `build.gradle.kts`: `koin.compose` → `koin.compose.multiplatform`. `DESIGN_SYSTEM.md` share-card paragraph updated (share is now part of the capture contract). 89 sources → `commonMain`, 48 MockK tests → `androidHostTest` (772 pass); all iOS targets compile; `ktlintCheck`/`styleCheck`/`assembleDebug` green |
| P5 | `feature:scan` | ✅ done — Android-only; iOS deferred (no real scanner). All 20 sources were already in `commonMain` and the 3 MockK action tests in `androidHostTest`; the finish work was the two `commonMain` files still touching Android APIs. The `BarcodeScanner` composable was already an `expect`/`actual` in `core:designsystem` (CameraX+MLKit on Android, "not available" stub on iOS), so the only remaining seam was the camera-permission gate: the platform-agnostic `CameraPermissionRequester` class (holds only a launch lambda + granted flag) stayed `commonMain`, and three `@Composable expect` funcs split out the platform pieces — `isCameraAvailable()` (Android `PackageManager.hasSystemFeature(FEATURE_CAMERA_ANY)`, iOS `false`), `isCameraPermissionGranted()` (Android `ContextCompat.checkSelfPermission(CAMERA)`, iOS `false`), `rememberCameraPermissionRequester(onResult)` (Android `ActivityResultContracts.RequestPermission()`, iOS a no-op requester that never grants). Because `isCameraAvailable()` is `false` on iOS the screen opens straight into manual ISBN entry — it compiles **and** runs as a manual-entry screen, the same graceful-degradation contract as the `BarcodeScanner` iOS stub. Two call-site fixups in `BarcodeScannerScreen`: dropped `LocalContext`/`PackageManager`, and hoisted `isCameraPermissionGranted()` out of the `remember {}` seed (now a `@Composable`, can't be invoked inside the `remember` lambda). `build.gradle.kts`: `koin.compose` → `koin.compose.multiplatform`. No `orchestration`/Koin change (`scanModule` + `BarcodeScannerScreen()` unchanged). All iOS targets compile; `:feature:scan:check`, root `ktlintCheck`/`styleCheck`, and `:app:assembleDebug` green |
...
| P5 | `feature:app_update` | ✅ done — the most Android-bound feature (a thin wrapper over Play's in-app-update API, no iOS equivalent, no UI). Domain (repo + 4 use cases), `AppUpdateRepositoryImpl` (pure delegation), the `AppUpdateDataSource` interface, the shared `NoOpAppUpdateSimulator`, `appUpdateModule`, and `expect val platformAppUpdateModule` → `commonMain`; `AppUpdateDataSourceImpl` (Play `AppUpdateManager` + install listener), `DebugAppUpdateSimulator`, the android `actual` module → `androidMain`; iOS binds a no-op `IosAppUpdateDataSource` (always `Idle`) + `NoOpAppUpdateSimulator` so `feature:settings`' `koinInject<AppUpdateSimulator>()` resolves on iOS. **Two notable problems.** (1) The domain leaked AndroidX — `startUpdateFlow(launcher: ActivityResultLauncher<IntentSenderRequest>)` ran through repo + use case. Resolved with an `AppUpdateFlowLauncher` **behavioural seam** (`fun interface { fun launch() }`, the `CameraPermissionRequester`/`ReadingSessionLauncher` idiom): the Android `actual` (`AndroidAppUpdateFlowLauncher`, built via Koin `parametersOf(updateLauncher)` in `MainActivity`) closes over the launcher + manager and **re-requests** the `AppUpdateInfo` (Play's documented pattern) to drive `startUpdateFlowForResult` — no cast, no AndroidX in `commonMain`. This made the data source's `cachedUpdateInfo` dead state, so it was dropped. (2) First migrated module with **build-type source sets** (`src/debug`/`src/release` = fake vs. real manager + simulator). A KMP library is single-variant, so those can't stay. Rather than push the split to `:app` (forbidden: the module graph bars `:app → feature`) or to `:orchestration` (it's KMP-bound next, P6 — would just recur), the debug/release decision became a **runtime `ApplicationInfo.FLAG_DEBUGGABLE` check inside `platformAppUpdateModule.android`**: debuggable builds get `FakeAppUpdateManager` + `DebugAppUpdateSimulator`, signed builds get the real manager + `NoOpAppUpdateSimulator`. Fully self-contained — no variant module, no `:orchestration`/`:app` wiring, survives orchestration's own KMP migration. MockK tests → `androidHostTest` (`StartAppUpdateFlowUseCaseTest` retargeted at the seam; data-source/repo `startUpdateFlow` cases dropped). `build.gradle.kts`: `softcover.android.library` → `softcover.kmp.library`, Play + `koin.android` → `androidMain`. All iOS targets compile; `:feature:app_update:check`, `checkModuleGraph`, `ktlintCheck`/`detekt`/`styleCheck`, and `:app:assembleDebug`/`assembleRelease` green |
| P6 | `:orchestration` | ✅ done (module conversion only — the iOS/desktop entry shell is the next row, deliberately deferred). `softcover.android.library`/`.compose` → `softcover.kmp.library`/`.compose`; deps split into `commonMain` (all core/feature projects + `koin.compose.multiplatform` + Voyager) and `androidMain` (`koin.android` + `androidx.splash`). The big move was carving the root composable out of `MainActivity`: the whole `setContent {}` body (theme, the `key(authenticated)` `RootScreen`↔`OnboardingScreen` navigator swap, the snackbar host, the app-update + `UserMessageNotifier` wiring, `ClearFocusOnTapScreen`) became a **shared `App()` in `commonMain`**, ready for a future iOS `MainViewController` to host. `MainActivity` (androidMain) shrank to the Android-only chrome — splash, edge-to-edge, the Focus-Mode launcher-intent handling, and the `onResume` update check — then just `setContent { App() }`; its `MainActivityViewModel` injection moved `by viewModel()` → `by inject()` (the VM is already a `commonMain` Koin `single`, so `App()` reaches it via `koinInject`). `RootScreen`/`BottomBarScreen`/`BottomNavigationBar` were pure `commonMain` lifts (every Compose API they use — `SharedTransitionLayout`, Voyager `ScreenTransition`, M3-expressive `HorizontalFloatingToolbar`, `WindowInsets`, `rememberSaveableStateHolder` — already proven CMP-safe). `AppNavigatorImpl` + the two cross-feature use-case impls + `SoftcoverModules` were pure `commonMain` moves. **Two seams.** (1) `AppEntryPoint` lives in `core:designsystem/androidMain` (it builds an `Intent`) and its only consumer is `feature:session`'s service (androidMain), so its binding moved out of `orchestrationModule` into a new `expect val platformOrchestrationModule` (pulled in via `includes(...)`): Android binds `AppEntryPointImpl`, iOS binds an **empty module** (no `AppEntryPoint` on iOS). (2) The app-update `ActivityResultLauncher` is Compose-scoped Android API that can't enter `commonMain` — wrapped in an orchestration-local **`internal @Composable expect fun rememberAppUpdateFlowLauncher()`** (Android: `rememberLauncherForActivityResult` + Koin `parametersOf`; iOS: a logged no-op, since update state is always `Idle` there). `internal` keeps the `app_update` type out of orchestration's ABI (so `app_update` stays an `implementation` dep, not `api`). The 3 MockK/JUnit5 tests → `androidHostTest`. Manifest (`MainActivity` + LAUNCHER) → `androidMain`. Repo-wide fix: added the CMP `org.jetbrains.compose.*` coordinates to the root `buildHealth` `onUnusedDependencies` exclude list (they're convention-plugin-provided, same rationale as the already-excluded `androidx.compose.*` ones — the list had never been updated for the CMP coordinates, so every migrated compose module was tripping the gate). Both iOS targets compile; `:orchestration:check`, `checkModuleGraph`, relocated tests, and `:app:assembleDebug` green; `:orchestration` is `buildHealth`-clean. |
| P6 | iOS entry point (`:orchestration` framework + `iosApp/` shell) | ✅ done (Kotlin side) — `:orchestration` exports a **static `OrchestrationKit` framework** (`iosArm64` + `iosSimulatorArm64`; `binaries.framework` in the module build, not the convention plugin, so only the aggregator ships a framework). A shared **`initKoin(platformDeclaration)`** seam (orchestration `commonMain`) replaced the Android-baked `startKoin`; Android `SoftCoverApp` now passes `androidContext` + `appModule`/`debugRoutesModule` through it. iOS `iosMain` adds `MainViewController()` (`ComposeUIViewController { App() }`), `initKoinIos()`, an `iosAppModule` (the iOS counterpart of `:app`'s `appModule`: `IosAppVersionProvider` reading `NSBundle`, `NotificationAppearance` from the empty iOS tokens, a no-op `DebugRoutesContent`), and the **Coil-on-iOS** fix (OkHttp is JVM-only and Apollo brings no Ktor, so `coil-network-ktor3` + `ktor-client-darwin` wired into `iosMain` only, used via `KtorNetworkFetcherFactory` in `MainViewController`; **`ktor` pinned to `3.1.0` to match the version `coil-network-ktor3:3.2.0` forces transitively** — a 3.0.3 pin left `ktor-client-darwin` behind `ktor-utils`/`-http` at 3.1.0 and `readHeaders` hit an `IrLinkageError` on the first image request, the same compile-vs-link skew class as the datetime one). Post-Koin startup wiring was extracted into a shared **`startAppServices(koin)`** in `commonMain` (installs `NetworkAvailability`, starts the offline write-sync queues, warms the user id) so Android `SoftCoverApp` and iOS `initKoinIos` can't drift; only the Android-only `NotificationChannelInitializer.initialize()` stays in `SoftCoverApp`. Two **device-runtime** gotchas the simulator surfaced that no gate catches (debug-only, post-link): (1) Compose Multiplatform's `PlistSanityCheck` aborts at launch unless **`CADisableMinimumFrameDurationOnPhone = true`** is in the real `Info.plist` — it can't be supplied via an `INFOPLIST_KEY_*` build setting (Xcode silently drops keys it doesn't recognise), so the `iosApp` target uses a physical `Info.plist` (`GENERATE_INFOPLIST_FILE = NO`); (2) the Ktor/datetime skews only throw when the linked binary actually *runs* the affected path. `compileKotlinIosSimulatorArm64` + `linkDebugFrameworkIosSimulatorArm64` green (the latter needs full Xcode — `DEVELOPER_DIR=/Applications/Xcode.app/Contents/Developer` if `xcode-select` points at the CLT). The Swift shell (`iosApp/`: `iOSApp.swift`, `ContentView.swift`, `Info.plist`, `README.md`) is scaffolded and the **`iosApp.xcodeproj` now exists** (committed): the `iosApp` target builds against the exported framework, uses a physical `Info.plist` (`GENERATE_INFOPLIST_FILE = NO`), and ships the **Softcover launcher icon** (a flattened 1024² PNG composited from the Android adaptive-icon layers, no alpha) + the **`Softcover` display name** (`CFBundleDisplayName`). **Remaining human step: code-signing + running on a device/simulator** (see `iosApp/README.md`). **The first real iOS link also surfaced a latent foundation break — see the kotlinx-datetime row below.** |
| P6 | kotlinx-datetime 0.6.2 → 0.7.1 (root-cause fix) | ✅ done — the iOS framework link (the **first** real iOS binary in the repo) exposed that the expressive `material3:1.11.0-alpha07` alpha **transitively forces kotlinx-datetime 0.7.1** on the Native graph, while `core:domain`/`book`/`personal`/`lists`/`deadlines`/`preferences`/`profile` were compiled against the pinned 0.6.2 — so ~28 `kotlinx.datetime.Clock`/`Instant` references across 7 modules were **unlinked symbols** (in 0.7.1 `Clock`/`Instant` moved to `kotlin.time`), which Kotlin/Native partial-linkage defers to runtime `IrLinkageError` crashes. **The per-module `check` gate never caught it because it only runs `compileKotlinIos*` (klib compilation in isolation, each module against its own resolved 0.6.2); the version conflict only materialises when a CMP module and a non-CMP module are linked into one binary.** Fix: pin the catalog to **0.7.1** and migrate the imports to `kotlin.time.Clock`/`kotlin.time.Instant` (the `todayIn`/`toLocalDateTime` extensions stay in `kotlinx.datetime`). This made the `currentLocalDate`/`currentLocalDateTime`/`currentInstant` **seam redundant** — both platforms now use the same `Clock` — so the three `CurrentDate.*` files collapsed into one plain `commonMain` implementation (the `expect`/two `actual`s deleted). Android `:app:assembleDebug`, both iOS targets compile **and link clean** (zero partial-linkage warnings), `ktlintCheck`/`detekt`/`styleCheck` and all touched-module `androidHostTest` suites green. |
| P6 | `:app` (stays Android shell) + desktop (JVM) entry point | ✅ **Done.** Phase 1: `jvm()` added centrally to `KmpLibraryConventionPlugin` (so every KMP module gains the target), `kspJvm` added to `AndroidRoomConventionPlugin` (Room generates the JVM `SoftcoverDatabaseConstructor`), a JVM-compile `check` gate added to the root build (host-agnostic, unlike the macOS-only iOS gate), and a **`:desktopApp`** module (Kotlin/JVM + `org.jetbrains.compose`, `compose.desktop.application`, `main()` → `application { Window { DesktopApp() } }`). `:orchestration/jvmMain` adds the desktop entry trio — `DesktopApp()` (installs the Coil OkHttp fetcher, same engine as Android), `initKoinDesktop()`, `desktopAppModule` (`JvmAppVersionProvider` reading the jar manifest, empty notification tokens, no-op `DebugRoutesContent`) — mirroring the iOS `MainViewController`/`initKoinIos`/`iosAppModule` trio. A `jvmMain` `actual` exists for all ~31 seams: the mechanical no-decision ones are **real** (`formatGrouped/DecimalNumber`→`java.text.NumberFormat`, `ClipboardReader`→AWT, `Haptics`→no-op, `playDecorativeMotion`→true, `TransientNavArg`→`kotlin.jvm.Transient` like Android, dynamic-color→null/`false`, `localImageSourceOrNull`→`java.io.File`); the platform-N/A ones stay no-ops matching iOS (app-update, lock-screen session launcher, dynamic color, haptics, barcode/camera scanning). Repo-wide `buildHealth` fix: the Compose-plugin-injected `jvmDev` desktop runtime (`desktop-jvm-<os>`) + `hot-reload-runtime-api` added to the `onUnusedDependencies` exclude list (same rationale as the CMP coordinates already there); two genuinely-unused `:app` deps (`core:identity`, `core:connectivity`) removed (surfaced when the new jvm variant perturbed DA). `compileKotlinJvm` across the whole graph + `:desktopApp:compileKotlin` + `ktlintCheck`/`detekt`/`buildHealth`/`checkModuleGraph`/`styleCheck` + Android `:app:compileDebugKotlin` all green. **Phase 2 progress (each its own check-in):** **✅ storage** — a shared `desktopAppDataDirectory()` helper in `core:domain/jvmMain` (per-OS app-data dir: macOS `~/Library/Application Support/Softcover`, Windows `%APPDATA%`, Linux XDG; created on demand) backs the real database, settings/profile/search-history DataStores, and edition-image storage (mirroring the iOS actuals). **✅ secure storage** — `JvmSecureApiKeyStorage` (core:preferences/jvmMain) wraps **KSafe** (`eu.anifantakis:ksafe`, jvmMain-only, `appNamespace = "nl.rhaydus.softcover"`), which custodies the encryption key in the OS secret store (macOS Keychain / Windows DPAPI / Linux Secret Service) with a software fallback; `:desktopApp` packaging adds `modules("jdk.unsupported", "java.management")` for OS-backed custody in trimmed distributables. Also added `kotlinx-coroutines-swing` to `:desktopApp` (provides `Dispatchers.Main` on the AWT/Swing thread — the desktop counterpart of `coroutines-android`). **The desktop app launches** — runtime-verified end to end. **✅ connectivity** — `ConnectivityDataSourceImpl` polls reachability (a short socket probe) and flips `isOnline` (the JDK has no network-change callback). **✅ notifications** — `JvmSoftcoverNotifier` posts via AWT `SystemTray`/`TrayIcon`. **✅ share-card** — Skiko PNG encode (mirroring iOS) + `JFileChooser` save + system-clipboard share. **✅ fonts** — Fraunces + Inter bundled as `composeResources` and the typography made composable, so **all three platforms** render the branded editorial type; the per-platform `Type` seam and the Android google-fonts dependency were removed (collapsed to one shared `commonMain` impl). The desktop target is now at functional parity with iOS; the remaining stubs (barcode/camera, app-update, lock-screen session) are platform-N/A and match iOS. |
