...
> its Softcover launcher icon + display name wired. **The only remaining work is the deferred desktop (JVM)
> entry point** (§7, last row) — a full new-target bring-up, deliberately not started. Update the
...
| P6 | `:app` (stays Android shell) + desktop (JVM) entry point | ☐ — desktop is a **full new-target bring-up** (add `jvm()` to every KMP module via the convention plugins, `kspJvm` for Room, a `jvmMain` actual for all ~31 seams incl. an OS-keychain `SecureApiKeyStorage`, fonts, share-card; plus a `:desktopApp` module), not just a shell. Deferred to its own phase. |
