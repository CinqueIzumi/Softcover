...
class OnShowEditionSheetClickActionTest {
    ...
    inner class Execute {
        ...
        fun `sets bookToUpdate to the provided book`() = runTest {
            // ----- Arrange -----
            val book = stubBook()
            val action = OnShowEditionSheetClickAction(book = book)
        ...
        @Test
        fun `sets showEditionSheet to true`() = runTest {
            // ----- Arrange -----
            val book = stubBook()
            stateFlow.value = ReadingScreenUiState(showEditionSheet = false)
            val action = OnShowEditionSheetClickAction(book = book)
        ...
        fun `replaces previously stored bookToUpdate with the new book`() = runTest {
            ...
            val action = OnShowEditionSheetClickAction(book = newBook)
