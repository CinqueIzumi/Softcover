...
class WantToReadBooksCollectorTest {
    ...
    inner class OnLaunch {
        ...
        @Test
        fun `updates wantToReadBooks and clears isLoading when flow emits`() = runTest(UnconfinedTestDispatcher()) {
            // ----- Arrange -----
            val books = listOf(mockk<Book>(), mockk<Book>())
            val collector = WantToReadBooksCollector()
            ...
            // ----- Assert -----
            stateFlow.value.wantToReadBooks shouldBe books
            stateFlow.value.isLoading shouldBe false
            job.cancel()
        }

        @Test
        fun `updates wantToReadBooks to the latest emission when flow emits multiple times`() = runTest(UnconfinedTestDispatcher()) {
            // ----- Arrange -----
            ...
            // ----- Assert -----
            stateFlow.value.wantToReadBooks shouldBe secondBooks
            job.cancel()
        }

        @Test
        fun `updates wantToReadBooks to empty list when flow emits empty list`() = runTest(UnconfinedTestDispatcher()) {
            // ----- Arrange -----
            val collector = WantToReadBooksCollector()
            val job = launch { collector.onLaunch(scope = scope, dependencies = dependencies) }
            ...
            // ----- Assert -----
            stateFlow.value.wantToReadBooks shouldBe emptyList()
            stateFlow.value.isLoading shouldBe false
            job.cancel()
        }

        @Test
        fun `does not change wantToReadBooks before the flow emits`() = runTest(UnconfinedTestDispatcher()) {
            // ----- Arrange -----
            val collector = WantToReadBooksCollector()
            val job = launch { collector.onLaunch(scope = scope, dependencies = dependencies) }

            // ----- Act & Assert -----
            stateFlow.value.wantToReadBooks shouldBe null
            job.cancel()
        }

        @Test
        fun `preserves other state fields when updating wantToReadBooks`() = runTest(UnconfinedTestDispatcher()) {
            // ----- Arrange -----
            val existingAllBooks = listOf(mockk<Book>())
            stateFlow.value = LibraryUiState(allBooks = existingAllBooks, isLoading = true)
            ...
            // ----- Assert -----
            stateFlow.value.allBooks shouldBe existingAllBooks
            stateFlow.value.wantToReadBooks shouldBe books
            job.cancel()
        }
    }
