 ...
 * every search/filter/sort input. [DisplayListsCollector] maps state to this bundle and gates
...
internal data class DisplayInputs(
    ...
    /**
     * Runs the search + filter (+ edition sort) derivation for every tab. Year-finished narrowing is
     * one more facet inside [LibraryFilters] (see [LibraryFilters.readYear]) rather than a separate
     * input. [prevBooksByTab] / [prevEditionsByTab] carry the previous emission so a tab whose
     * recomputed list is structurally equal reuses the prior instance — keeping downstream
     * `remember(list)` blocks from invalidating when nothing about that tab actually changed.
     */
    fun compute(
