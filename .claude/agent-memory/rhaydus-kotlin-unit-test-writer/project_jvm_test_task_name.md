...
Also confirmed for `:core:designsystem` (writing `CoverlessTitleMetricsTest`): a prompt-suggested `testDebugUnitTest` task name also does not exist for this module ("task 'testDebugUnitTest' not found") — `testAndroidHostTest` is the one that works. When any suggested task name 404s, run `./gradlew :<module>:tasks --all | grep -i AndroidHostTest` to confirm the real name rather than guessing further variants.
