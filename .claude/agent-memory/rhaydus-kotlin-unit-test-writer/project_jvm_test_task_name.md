...
**Why:** the module's `androidHostTest` source set isn't wired to a task literally named `jvmTest`, even though CLAUDE.md's example commands reference `./gradlew test` / `:app:test`. This was hit independently while writing tests for `ReadingPaceForecast`, `ReadingJournalEntryMapper`, and `OnLensSelectedAction` in `:core:domain` and `:feature:book_detail`.
